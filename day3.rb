mappy = {}
("a".."z").each_with_index { |f, index| mappy[f] = index+1 }
("A".."Z").each_with_index { |f, index| mappy[f] = index+27 }
...
rucksacks.each do |sack|
  ...
  first = sack[0, half]
  second = sack[half, leng-1]

  dude = first.split("")&second.split("")
  puts "first: #{first}, second: #{second}, dude: #{dude[0]}"
  sum += mappy[dude[0]]
end
...
sum = 0
rucksacks.each_with_index do |sack, index|
  next if (index+1) % 3 != 0
  sack2 = rucksacks[index-1]
  sack3 = rucksacks[index-2]
  dude = sack.split("")&sack2.split("")&sack3.split("")
  sum += mappy[dude[0]]
end
...
rucksacks = ["mmbclcsDHCflDDlCrzzrDWjPJvjPvqJPjfpqZQdfWd",
